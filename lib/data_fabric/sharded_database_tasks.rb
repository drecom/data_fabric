...
module ActiveRecord
  module Tasks
    module DatabaseTasks
      ...
      self.exclude_shard_regexp = /(master|slave|standby)/
      ...
      def drop_current_cluster(environment = env)
        each_current_cluster_configuration(environment) { |configuration|
          puts "*** executing to database: #{configuration['database']}"
          drop configuration
        }
      ...
      def each_current_cluster_configuration(environment, exclude_pattern = exclude_shard_regexp)
        environments = [environment]
        environments << 'test' if environment == 'development'
        regexp = Regexp.compile("(#{environments.map {|e| Regexp.escape(e)}.join('|')})")

        configurations = ActiveRecord::Base.configurations.select {|k,v| k =~ regexp and (!exclude_pattern or k !~ exclude_pattern) }.values.uniq
